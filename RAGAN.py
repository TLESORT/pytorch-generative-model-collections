    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter, (x_, _) in enumerate(self.data_loader):
                ...
                # Discriminator loss
                D_loss = (BCE_stable(y_pred - torch.mean(y_pred_fake), self.y_real_) + BCE_stable(
                    torch.mean(y_pred_fake) - y_pred, self.y_fake_)) / 2
                D_loss.backward()
                ...
                G_loss = (BCE_stable(y_pred - torch.mean(y_pred_fake), self.y_fake_) + BCE_stable(
                    torch.mean(y_pred_fake) - y_pred, self.y_real_)) / 2
                G_loss.backward()
